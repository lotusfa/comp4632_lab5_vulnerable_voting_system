...
<head>
    ...
    <script type="text/javascript">
        function validateVote() {
            console.log('hi');
            var errmsg = "";
            if (!validateRadioButton('radio_option')) {
                errmsg += "Must check some radio_option!<br />";
            }

            if (!validateRadioButton('radio_topic')) {
                errmsg += "Must check some radio_topic!<br />";
            }
    ...
    <form id='form_vote' name='form_vote' method='POST' action='voting.php?login=<?=$email;?>'>
        ...
        <table border='0'>
            ...
            for ($i=0; $i<count($rows); $i++) {
                ...
                ?>
                <tr bgcolor='#C1E0EB'>
                    <td><input type='RADIO' id='radio_topic' name='radio_topic'
                        value='<?=$row["_id"];?>'></td>
                        <td><?=$row["topic"];?></td>
                        <td><?=$row["option_a"];?></td>
                        <td><?=$row["option_b"];?></td>
